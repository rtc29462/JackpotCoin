...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    ...
    {
        const CWalletTx& wtx = (*it).second;
        if (wtx.IsCoinBaseOrStake() || !wtx.IsFinal())
            continue;
...
static CScript _createmultisig(const Array& params)
{
    ...
    for (unsigned int i = 0; i < keys.size(); i++)
    {
        ...
        // Case 1: JackpotCoin address and we have full public key:
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; JackpotCoin server stopping, restart to run with encrypted wallet.  The keypool has been flushed, you need to make a new backup.";
}
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <jackpotcoinaddress>\n"
            "Return information about <jackpotcoinaddress>.");
...
Value validatepubkey(const Array& params, bool fHelp)
{
    if (fHelp || !params.size() || params.size() > 2)
        throw runtime_error(
            "validatepubkey <JackpotCoinpubkey>\n"
            "Return information about <JackpotCoinpubkey>.");
...
// JackpotCoin: resend unconfirmed wallet transactions
