...
Value getrawtransaction(const Array& params, bool fHelp)
{
    ...
    if (!tx.GetBlockEx(hash, block, tx, txOffset))
...
Value listunspent(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 2)
    {
        ...
        BOOST_FOREACH(Value& input, inputs)
        {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid JackpotCoin address: ")+input.get_str());
...
Value sendrawtransaction(const Array& params, bool fHelp)
{
    ...
    CCoins existingCoins;
    {
        ...
        if (!fHave) {
            ...
            if (!tx.AcceptToMemoryPool(state, true, NULL))
