...
// Copyright (c) 2014-2014 Jackpotcoin developers
// Distributed under the MIT/X11 software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.
#ifndef BITCOIN_REWARD_H
#define BITCOIN_REWARD_H
...
static const int64 POW_REWARD_MAXIMUM         = 100000 * COIN;
static const int64 POW_REWARD_MININUM         = 1 * COIN;
static const int   POW_REWARD_REDUCE_BLOCKS   = 1000;
static const int   POW_REWARD_REDUCE_FACTOR   = 90;
...
/** The minimum amount for TX FEE */
static const int64 MIN_TX_FEE = 1 * CENT;
/** The minimum amount for TXOUT */
...
static const int MATURITY_COINBASE                = 60;
...
uint64 UInt64DeComp(uint64 n);


bool GetWalletFile(CWallet* pwallet, std::string &strWalletFileOut);

/** Position on disk for a particular block. */
struct CDiskBlockPos
{
    int nFile;
    unsigned int nPos;
...
struct CDiskTxPos : public CDiskBlockPos
{
    ...
    std::string ToString() const
    {
        if (IsNull())
            ...
            return strprintf("(nFile=%u, nPos=%u, nTxOffset=%u)", nFile, nPos, nTxOffset);
    }
...
public:
    ...
    unsigned int nPoWLast;    // 24 24bit last height from the current hight 
    unsigned int nPoSPot;     // 32 Number of PoS coins in POT, only higher than 1 coin.  
    unsigned int nPoSLast;    // 24 24bit last height 
    unsigned int nPoSBet;     // 8bit BET 0 ~ 200
    ...
    */
    ...
    // Read BlockIndex using TxID
    bool GetBlockIndex(const uint256 &hash, CBlockIndex * &blockindex) const;
    
    // Load Full Block using TxID
    bool GetBlock(const uint256 &hash, CBlock &block) const;

    // Load Full Block and Transaction/Offset using TxID
    bool GetBlockEx(const uint256 &hash, CBlock &block, CTransaction &txOut, unsigned int &txOffset) const;

    // Read Transaction from Memory or Disk
    bool GetTransaction(const uint256 &hash, CTransaction &txOut) const;
...
public:
    ...
    std::string ToString() const
    {
        ...
        str += strprintf("(height = %d, nVersion=%d, vout.size=%"PRIszu")\n",
            nHeight,
            nVersion,
            vout.size());
...
public:
    ...
    (
        ...
        READWRITE(nNonce);
        if (nType & SER_DISK) {
           READWRITE(VARINT(nSuperBlock));
           READWRITE(VARINT(nRoundMask));
        }
        else {
...
public:
    ...
    CBlockHeader GetBlockHeader() const
    {
        ...
        blockheader.nSuperBlock    = nSuperBlock;
        blockheader.nRoundMask     = nRoundMask;
        return blockheader;
    }
    ...
    void print() const
    {
        printf("CBlock(hash=%s, ver=%d, hashPrevBlock=%s, hashMerkleRoot=%s, nTime=%u, nBits=%08x, nNonce=%u, nSuperBlock=%u, nRoundMask=%u,  vtx=%"PRIszu", vchBlockSig=%s)\n",
            GetHash().ToString().c_str(),
            nVersion,
...
public:
    ...
    std::string ToString() const
    {
        return strprintf("CBlockIndex(pprev=%p, pnext=%p, nFile=%u, nDataPos=%-6d nHeight=%d, nMoneySupply=%s, nFlags=(%s)(%d)(%s), nStakeModifier=%016"PRI64x", nStakeModifierChecksum=%08x, hashProofOfStake=%s, prevoutStake=(%s), nStakeTime=%d merkle=%s, hashBlock=%s)",
