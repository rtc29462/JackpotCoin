...
bool CDBEnv::Open(boost::filesystem::path pathEnv_)
{
    ...
    if (!dbenv.get_lk_max_locks(&nMaxLocks))
    {
        ...
        {
            if (nBlocks < 1)
                strMessage = strprintf(("Warning: DB_CONFIG has set_lk_max_locks %lu, which may be too low for a single block. If this limit is reached, NovaCoin may stop working."), (unsigned long)nMaxLocks);
            else
                strMessage = strprintf(("Warning: DB_CONFIG has set_lk_max_locks %lu, which may be too low for a common blockchain reorganization. If this limit is reached, NovaCoin may stop working."), (unsigned long)nMaxLocks);
...
bool CTxDB::LoadBlockIndexGuts()
{
    ...
    while (true)
    {
        ...
        if (strType == "blockindex" && !fRequestShutdown)
        {
            ...
            pindexNew->nSuperBlock    = diskindex.nSuperBlock;
