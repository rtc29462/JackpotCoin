...
bool CDBEnv::Open(const boost::filesystem::path& pathIn)
{
    ...
    dbenv.set_cachesize       (0, 0x200000, 1);
    dbenv.set_lg_bsize        (0x100000);
    dbenv.set_lg_max          (0x100000);
    dbenv.set_lk_max_locks    (0x100000);
    dbenv.set_lk_max_objects  (0x10000);
    ...
    int ret = dbenv.open(path.string().c_str(),
                     ...
                     S_IRUSR | S_IWUSR);
    if (ret != 0)
        return error("CDBEnv::Open() : error %s (%d) opening database environment", DbEnv::strerror(ret), ret);
    ...
    if (!dbenv.get_lk_max_locks(&nMaxLocks))
    {
        ...
        {
            if (nBlocks < 1)
                strMessage = strprintf(("Warning: DB_CONFIG has set_lk_max_locks %lu, which may be too low for a single block. If this limit is reached, JackpotCoin may stop working."), (unsigned long)nMaxLocks);
            else
                strMessage = strprintf(("Warning: DB_CONFIG has set_lk_max_locks %lu, which may be too low for a common blockchain reorganization. If this limit is reached, JackpotCoin may stop working."), (unsigned long)nMaxLocks);
